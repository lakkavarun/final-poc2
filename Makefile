...
THREAD_DEMO_TARGET = thread_demo
...
.PHONY: all debug run test test-cunit test-cunit-stub misra cppcheck valgrind helgrind logger \
        validate report O0 O1 O2 O3 release clean coverage optimize thread-demo
...
# =============================================================================
# 3b. Build all optimization levels at once + compare binary sizes.
#     Convenience wrapper around O0-O3 above -- useful for confirming the
#     flags are actually taking effect (binary size/behavior should differ
#     across levels) without four separate `make` invocations.
# =============================================================================
optimize: $(O0_TARGET) $(O1_TARGET) $(O2_TARGET) $(O3_TARGET)
	@echo ""
	@echo "Binary sizes by optimization level:"
	@ls -l $(O0_TARGET) $(O1_TARGET) $(O2_TARGET) $(O3_TARGET) | awk '{printf "  %-20s %8s bytes\n", $$NF, $$5}'
...
# =============================================================================
# 6b. Thread demo -- standalone program that spawns real pthreads doing
#     actual subdb_add/search/delete operations, each printing its own
#     kernel thread id (gettid()) as visible, interleaved proof of real
#     OS-level concurrency, cross-checked against /proc/self/task counts.
#     Links CORE_SRCS directly (no main.c/server.c), same pattern as
#     $(TEST_TARGET) above.
# =============================================================================
thread-demo: $(THREAD_DEMO_TARGET)
	./$(THREAD_DEMO_TARGET)

$(THREAD_DEMO_TARGET): tests/thread_demo.c $(CORE_SRCS)
	$(CC) $(CSTD) $(WARN) $(INC) tests/thread_demo.c $(CORE_SRCS) -o $(THREAD_DEMO_TARGET) $(LDLIBS)
...
# =============================================================================
# 6d. Code coverage (gcov/lcov)
#     Rebuilds tests/test_main.c + CORE_SRCS with --coverage (adds
#     -fprofile-arcs -ftest-coverage at compile time and links gcov).
#     Running the binary emits .gcda files next to the .o files. Make
#     always executes recipes with CWD = this Makefile's directory, so
#     relative paths like data/subscribers.csv resolve correctly here --
#     the earlier failure only happened when the binary was launched
#     manually from a different working directory (e.g. a plain VS Code
#     terminal not cd'ed into the project root). lcov then aggregates the
#     .gcda/.gcno files into coverage.info and genhtml renders the HTML
#     report. Requires lcov + genhtml on PATH (part of the lcov package);
#     if missing, install it (e.g. `apt install lcov`) -- not done here,
#     per the "does not install anything" rule at the top of this file.
# =============================================================================
coverage: $(COVERAGE_TARGET)
	./$(COVERAGE_TARGET)
	lcov --capture --directory . --output-file $(COVERAGE_INFO) --rc branch_coverage=1 --ignore-errors negative
	lcov --remove $(COVERAGE_INFO) "tests/*" --output-file $(COVERAGE_INFO)
	genhtml $(COVERAGE_INFO) --output-directory $(COVERAGE_HTML) --rc branch_coverage=1 --ignore-errors negative
	@echo "Coverage HTML report: $(COVERAGE_HTML)/index.html"

$(COVERAGE_TARGET): tests/test_main.c $(CORE_SRCS)
	$(CC) $(CSTD) $(WARN) $(INC) $(COVERAGE_FLAGS) tests/test_main.c $(CORE_SRCS) -o $(COVERAGE_TARGET) $(LDLIBS)
...
clean:
	rm -f $(TARGET) $(DEBUG_TARGET) $(RELEASE_TARGET) \
		...
		$(TEST_TARGET) $(CUNIT_TEST_TARGET) $(CUNIT_STUB_TARGET) $(COVERAGE_TARGET) \
		$(THREAD_DEMO_TARGET)
	rm -f $(MISRA_REPORT) $(CPPCHECK_REPORT) $(VALGRIND_REPORT) $(HELGRIND_REPORT) \
		$(LOGGER_REPORT) $(VALIDATION_REPORT) $(COVERAGE_INFO)
	rm -rf $(COVERAGE_HTML)
